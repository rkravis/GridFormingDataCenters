...
def run_ssm_set_scr(output_dir, case, system, name, scale_factor=[1.0]):
    os.makedirs(output_dir, exist_ok=True)
    # Get line object 
    for scale in scale_factor:
        system1 = copy.deepcopy(system)
        line = system1.lines[0]
        line.r_pu = line.r_pu * scale 
        line.x_pu = line.x_pu * scale 
        main.run_ssm(system=system1, case_directory=case)
        shutil.copy(case+"/outputs/small_signal_model/A.csv", output_dir+name+f'_A_{scale}.csv')
        shutil.copy(case+"/outputs/small_signal_model/B.csv", output_dir+name+f'_B_{scale}.csv')
...
def run_emt_set_src(output_dir, case, system, model_inputs, t_max, name, dc_string_name, scale=1.0):
    os.makedirs(output_dir, exist_ok=True)
    system1 = copy.deepcopy(system)
    line = system1.lines[0]
    line.r_pu = line.r_pu * scale 
    line.x_pu = line.x_pu * scale 
    main.run_emt(inputs=model_inputs, t_max=t_max, system=system1, case_directory=case)
    dc_files = [i for i in os.listdir(case+"/outputs/simulation_emt") if os.path.isfile(os.path.join(case+"/outputs/simulation_emt",i)) and dc_string_name in i]
    for f in dc_files:
        sfx = Path(f).suffix
        shutil.copy(case+"/outputs/simulation_emt/"+f, output_dir+name+sfx)
...
def make_overlay_plot(output_dir, plot_inputs:dict, title='', suffix=''):
    """ 
    ...
    for state, input in plot_inputs.items():
        fig = go.Figure()
        ...
        fig.write_image(os.path.join(output_dir,'figs',state+f'_overlay{suffix}.png'), width=600,height=600,scale=2)
        fig.write_image(os.path.join(output_dir,'figs',state+f'_overlay{suffix}.svg'), width=600,height=600,scale=3)
...
def add_power_calcs(df):

    i_bus_d = df.select('i_bus_d').to_numpy().flatten()
    i_bus_q = df.select('i_bus_q').to_numpy().flatten()
    v_sh_d = df.select('v_sh_d').to_numpy().flatten()
    v_sh_q = df.select('v_sh_q').to_numpy().flatten()
                
    p_sh = v_sh_d * i_bus_d + v_sh_q * i_bus_q
    q_sh = v_sh_q * i_bus_d - v_sh_d * i_bus_q
    df = df.with_columns(pl.Series('p_sh', p_sh))
    df = df.with_columns(pl.Series('q_sh', q_sh))
    return df
...
def read_all_sim_results(case_dir):
    ...
    gfli_a = add_power_calcs(gfli_a)
    gfli_e = add_power_calcs(gfli_e)
    gfli_d = add_power_calcs(gfli_d)
    gfmi_c = add_power_calcs(gfmi_c)
    gfmi_e = add_power_calcs(gfmi_e)

    return gfli_a, gfli_e, gfli_d, gfmi_c, gfmi_e, gfli_a_sh, gfli_e_sh, gfli_d_sh, gfmi_c_sh, gfmi_e_sh
...
def make_small_signal_plot(output_dir, inputs: list):
    ...
    fig.write_image(output_dir+f'/figs/max_sv_overlay.svg', width=1000,height=400)
...
def make_eigenvalue_comparison_plot(output_dir, inputs, pltname=''):
    ...
    for name in inputs:
        ...
        fig.add_trace(go.Scatter(x=re_eig, y=imag_eig, name=name, mode='markers', marker=dict(size=marker_size_map[name], color=color_map[name], opacity=0.7, symbol=marker_shape_map[name])))
        
    fig.update_xaxes(title_text='Real')
    fig.update_yaxes(title_text="Imag")    
    fig.update_yaxes(range=[-20,20])
    fig.update_xaxes(range=[-10,1])
    fig.add_hline(
    y=0, 
    line_width=1, 
    ...
    fig.write_image(output_dir+f'/figs/eigenvalues_overlaid_{pltname}.png',width=600,height=600,scale=2)
    fig.write_image(output_dir+f'/figs/eigenvalues_overlaid_{pltname}.svg', width=600, height=600)
        
   
   
def make_eigenvalue_comparison_plot_varying_scr(output_dir, inputs, scales, pltname=''):
    
    """ Plots eigenvalues of each system in inputs overlaid."""
    os.makedirs(os.path.join(output_dir, "figs/"), exist_ok=True)
    
    opacity_range = np.linspace(0.2, 1.0, len(scales))
    
    fig = go.Figure()
    for name in inputs:
        # extract A matrix for each scr 
        opacity_idx = 0
        for scale in scales:
            scr = np.round(1/np.sqrt((0.0001*scale)**2 + (0.001*scale)**2),0)
        
            A = pl.read_csv(output_dir+name+f"_A_{scale}.csv")
            Amat = A[:,1:].to_numpy()
            re_eig = np.linalg.eigvals(Amat).real 
            imag_eig = np.linalg.eigvals(Amat).imag 
            if opacity_range[opacity_idx] == 1:
                fig.add_trace(go.Scatter(x=re_eig, y=imag_eig, name=name+f': SCR {scr}', mode='markers', marker=dict(size=marker_size_map[name], color=color_map[name], opacity=opacity_range[opacity_idx], symbol=marker_shape_map[name]), legendgroup=legend_group_map[name]))
                

            elif opacity_range[opacity_idx] == 0.2:
                fig.add_trace(go.Scatter(x=re_eig, y=imag_eig, name=name+f': SCR {scr}', mode='markers', marker=dict(size=marker_size_map[name], color=color_map[name], opacity=opacity_range[opacity_idx], symbol=marker_shape_map[name]), legendgroup=legend_group_map[name]))
            else:
                fig.add_trace(go.Scatter(x=re_eig, y=imag_eig, showlegend=False, mode='markers', marker=dict(size=marker_size_map[name], color=color_map[name], opacity=opacity_range[opacity_idx], symbol=marker_shape_map[name])))
    ...
    fig.update_traces(
        marker=dict(
            line=dict(
                width=1,         # Thickness of the edge in pixels
                color='black' # Color of the edge
            )
        )
    )
    
    fig.write_html(output_dir+f'/figs/eigenvalues_overlaid_{pltname}_varying_scr.html')  
    ...
    fig.write_image(output_dir+f'/figs/eigenvalues_overlaid_{pltname}_varying_scr.png',width=800,height=600,scale=8)
    fig.write_image(output_dir+f'/figs/eigenvalues_overlaid_{pltname}_varying_scr.svg', width=800, height=600,scale=20)
    fig.write_image(output_dir+f'/figs/eigenvalues_overlaid_{pltname}_varying_scr.pdf', width=800, height=600,scale=3)
...
marker_shape_map = {'gfli_a': 'square',
             'gfli_d': 'circle',
             'gfli_e': 'circle',
             'gfmi_c': 'square',
             'gfmi_e': 'diamond' 
...
marker_shape_map = {'gfli_a': 'circle',
             'gfli_d': 'circle',
             'gfli_e': 'circle',
             'gfmi_c': 'circle',
             'gfmi_e': 'circle' 
